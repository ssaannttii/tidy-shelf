...
export default function Board() {
  ...
  // widest row (in shelf-cells) drives the fixed slot size so the cabinet never
  // reflows as items are cleared.
  const cols = Math.max(1, ...level.layout.map((r) => r.length));
  ...
  return (
    <div className="board-wrap">
      <div
        ...
        style={{ "--cols": cols, "--slots": slotsPer } as React.CSSProperties}
      >
